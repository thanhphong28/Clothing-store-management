...
public class ChiTietPhieuNhapDAO {
    ...
    public boolean isValidtoAdd(ChiTietPhieuNhapDTO ctpn) throws Exception{
        ...
        try{
            for (int i=0; i<arr.size(); i++){
                if (arr.get(i).getMaPN().equals(ctpn.getMaPN()) && arr.get(i).getMaHang().equals(ctpn.getMaHang())){
    ...
    public int suaCTPn(ChiTietPhieuNhapDTO ctpn){
        ...
        try{
            ...
            qry = qry +" where MaPn ='"+ctpn.getMaPN()+"' and MaHang='"+ctpn.getMaHang()+"'";
            res = my.executeUpdate(qry);
        }catch(Exception e){
    ...
    public int getSLct(String mapn,String maHang){
       int soluongct = 0;
       try{
           String qry = "select * from ctpn where MaPn='"+mapn+"' and MaHang='"+maHang+"'";
           ResultSet rs = my.executeQuery(qry);
           while(rs.next()){
