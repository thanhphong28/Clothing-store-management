...
public class ChiTietPhieuNhapBUS {
    ...
    public ArrayList<ChiTietPhieuNhapDTO> timkiem(String key, String query) {
        ...
        if (key.equals("Mã phiếu nhập")) {
            ...
            return temp;}
        if (key.equals("Mã hàng")) {
            for (int i = 0; i < ctpn.size(); i++) {
                if (ctpn.get(i).getMaHang().toLowerCase().equals(query)) {
                    temp.add(ctpn.get(i));
                }
    ...
    public Vector loadcb_hang(){
        ChiTietPhieuNhapDAO data = new ChiTietPhieuNhapDAO();
        Vector cbMa= data.docMaHang();
        return cbMa;
    }
    public int getSLct(String mapn,String mahang){
        ChiTietPhieuNhapDAO data = new ChiTietPhieuNhapDAO();
        int check = data.getSLct(mapn, mahang);
        return check;
    }
